...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  displacement = final_position - node.pose[:2]
  theta = node.pose[YAW]
  ...
  final_node.pose[YAW] = alpha + beta

  centre, radius = find_circle(node, final_node)
  #print(centre, radius)

  arc_angle_1 = -np.pi/2 + theta 			# angle from centre of circle to start position
  arc_angle_2 = -np.pi/2 + final_node.pose[YAW] 	# angle from centre of circle to end position
  ...
  final_node.cost = arc_cost

  if node is not None:
      final_node.cost += node.cost
...
def rrt(start_pose, goal_position, occupancy_grid):
  ...
  for end in possible_ends:
    ...
    if end.cost < cost:			# Choose the shortest trajectory
      cost = end.cost
      final_node = end
