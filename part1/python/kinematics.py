...
def rk4(current_pose, t, dt):
  ...
  #Start with theta (YAW):
  k1_t = w
  k2_t = np.cos(t + dt/2)
  k3_t = np.cos(t + dt/2)
  k4_t = np.cos(t + dt)
  next_pose[YAW] = current_pose[YAW] + (dt*(k1_t+2*k2_t+2*k3_t+k4_t))/6
  
  theta_mid = current_pose[YAW] + (dt/2)*(k2_t)  # Computed using Euler Method

  #Now we compute  x
  k1_x = u*np.cos(current_pose[YAW])
  k2_x = u*np.cos(theta_mid)
  k3_x = k2_x
  k4_x = u*np.cos(next_pose[YAW])
  next_pose[X] = current_pose[X] + (dt*(k1_x +2*k2_x +2*k3_x + k4_x))/6

  #Finally we compute y
  k1_y = u*np.sin(current_pose[YAW])
  k2_y = u*np.sin(theta_mid)
  k3_y = k2_y
  k4_y = u*np.sin(next_pose[YAW])
