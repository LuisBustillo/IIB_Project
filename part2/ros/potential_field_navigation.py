...
def get_relative_position(absolute_pose, absolute_position):
  ...
  #absolute_position = [x, y] for point P
  #absolute_pose = [X, Y, theta] for robot	
	
  displacement = absolute_position - absolute_pose[:2]
  ...
  relative_position = np.array([np.dot(displacement, x_unit), np.dot(displacement, y_unit)])
  #relative_position = np.arctan2(displacement[Y], displacement[X])
