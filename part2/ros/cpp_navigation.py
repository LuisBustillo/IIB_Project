...
# Import the rrt.py and cpp.py code rather than copy-pasting.
...
def feedback_linearized(pose, velocity, epsilon):
  ...
  u = xp_dot*np.cos(pose[YAW]) + yp_dot*np.sin(pose[YAW])
  
  w = (1/epsilon) * (-xp_dot*np.sin(pose[YAW]) + yp_dot*np.cos(pose[YAW]))
...
def get_velocity(position, path_points):
  ...
  closest = [999,999]			# Initializes a reference point far away from trajectory
  second_closest = [999,999]		# Initializes a second reference point far away from trajectory
  
  # Identify the two points in path_points between which the robot is located		
  
  for point in path_points:		
  	distance = np.linalg.norm(position - point)
  	closest_distance = np.linalg.norm(position - closest)
  	second_closest_distance = np.linalg.norm(position - second_closest)
  	if distance < closest_distance:
  		closest = list(point)
  	elif distance < second_closest_distance:
  		second_closest = list(point)
  	else:
  		pass
  	
  closest_index = path_points.tolist().index(closest)
  second_closest_index = path_points.tolist().index(second_closest)
