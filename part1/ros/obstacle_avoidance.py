...
def braitenberg(front, front_left, front_right, left, right):
  ...
  # weights for speed -->  [fr, fr_l, fr_r, r, l]
  weights_u = [0., .2, .2, 0., 0.]
  bias_u = 0.2
  u_input = np.dot(sensor_vals, weights_u) 
  # Linear Combination
  u = u_input + bias_u
  # Non-linear Combination
  # u = np.tanh(u_input + bias_u)
  # u = bias_u - 1/(1+np.exp(u_input))
  # u = 1/(1+np.exp(-u_input))
  
  # weights for rotation -->  [fr, fr_l, fr_r, r, l]
  weights_w = [0, 0.5, -0.5, 0., 0.]
  bias_w = 0.
  w_input = np.dot(sensor_vals, weights_w) 
  # Linear Combination
  w = w_input + bias_w
  # Non-linear Combination
  # w = np.tanh(w_input + bias_w)
  # w = - 1/(1+np.exp(u_input)) + bias_w
  # w = 1/(1+np.exp(-w_input))
